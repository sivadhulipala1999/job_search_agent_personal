...
def generate_markdown_report(
    ...
    new_jobs = sorted(
        [j for j in jobs if j.get("status") == "New"],
        key=lambda x: x.get("score", 0),
        reverse=True,
    )
    repeated_jobs = sorted(
        [j for j in jobs if j.get("status") == "Repeated"],
        key=lambda x: x.get("score", 0),
        reverse=True,
    )
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        if not new_jobs:
            ...
            for i, job in enumerate(new_jobs, 1):
                f.write(
                    f"### {i}. {job.get('title', 'Unknown Title')} ({job.get('score', 0)}% Profile Match)\n"
                )
                ...
                f.write(
                    f"- **Description Snippet:** {job.get('description', '')[:300]}...\n\n"
                )
                f.write("---\n\n")
