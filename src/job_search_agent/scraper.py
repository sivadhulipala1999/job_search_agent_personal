...
def is_title_match(job_title: str, keywords: List[str]) -> bool:
    ...
    for kw in keywords:
        ...
        if any(token in title_lower for token in tokens if len(token) > 2): # ignore tiny words like 'AI' could be tricky but 'AI' is len 2... let's just do > 1
            return True
        # also check exact kw
...
def scrape_with_google_jobs(keyword: str, country: str, results_wanted: int) -> List[Dict[str, Any]]:
...
def scrape_with_jobspy(keyword: str, country: str, results_wanted: int) -> List[Dict[str, Any]]:
...
def scrape_jobs_for_keywords(keywords: List[str], country: str, results_wanted: int = 15) -> List[Dict[str, Any]]:
    ...
    for word in keywords:
        ...
        if not jobs:
            print(f"Google Jobs yielded 0 results for '{word}'. Engaging Legacy Fallback...")
