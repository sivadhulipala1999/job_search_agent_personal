...
class ApplicationMaterials(BaseModel):
    cv_recommendations: str = Field(
        description="Markdown formatted expert recruiter advice on exactly what CV lines to add/change/highlight to secure an interview."
    )
    cover_letter: str = Field(
        description="A highly targeted, professional cover letter ready for the candidate to use."
    )
...
def get_client_and_model() -> tuple[OpenAI, str]:
    ...
    if not api_key:
        raise ValueError(
            "Neither LLM_API_KEY nor OPENAI_API_KEY environment variables are set."
        )
...
def _call_llm_with_fallback(
    client: OpenAI, model: str, messages: List[Dict[str, str]], response_format: Any
) -> Any:
...
def expand_seed_keywords(
    seed_keywords: List[str], cv_text: str, num_expansions: int = 3
) -> List[str]:
    ...
    try:
        response = _call_llm_with_fallback(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a technical recruiter. Return a list of short job titles/keywords.",
                },
                {"role": "user", "content": prompt},
            ],
            response_format=KeywordExtraction,
    ...
    try:
        response = _call_llm_with_fallback(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert HR sourcer matching candidates to jobs.",
                },
                {"role": "user", "content": prompt},
            ],
    ...
    try:
        response = _call_llm_with_fallback(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are dual-role HR expert and professional ghostwriter. Ensure language matching.",
                },
                {"role": "user", "content": prompt},
            ],
