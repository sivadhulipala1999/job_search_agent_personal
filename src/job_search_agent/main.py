...
def main():
    ...
    if not country:
        print("Error: TARGET_COUNTRY expects an environment variable. Please define it in your .env or .env.example file.")
        sys.exit(1)
    ...
    if not pdf_files:
        print(f"Error: No PDF files found in '{cv_dir}'. Please paste your CV in that folder and run again.")
        sys.exit(1)
    ...
    if not seed_file.exists():
        ...
        try:
            user_input = input("Confirm these keywords or provide your own comma-separated list (Press Enter to keep defaults): ")
            if user_input.strip():
    ...
    jobs = scrape_jobs_for_keywords(all_keywords, country, results_wanted=results_per_kw)
    ...
    # 5. Score Jobs
    print(f"Extracted {len(jobs)} unique jobs (after local pre-filters). Scoring relevancy using LLM...")
    final_jobs = []
    
    for i, job in enumerate(jobs):
        sys.stdout.write(f"\rScoring [{i+1}/{len(jobs)}]: {job.get('title', '')[:40]:<40}")
        sys.stdout.flush()
    ...
    report_path = generate_markdown_report(all_keywords, final_jobs, filename=f"curated_list_{date_str}.md")
    ...
    if top_jobs:
        ...
        for i, job in enumerate(top_jobs, 1):
            ...
            sys.stdout.write(f"\rDrafting Application [{i}/{len(top_jobs)}]: {job.get('company', '')[:30]}")
            sys.stdout.flush()
            
            materials = generate_application_materials(cv_text, job.get("description", ""))
